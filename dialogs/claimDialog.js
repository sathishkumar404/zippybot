...
class claimDialog extends CancelAndHelpDialog {
  ...
  async policyNo(stepContext) {
   ...
   async getDOB(stepContext) {
    ...
    const bookingDetails = stepContext.options; 
    var dob ="Hi, could you please confirm your date of birth?";
    // Capture the results of the previous step
    bookingDetails.claimPolicyNo = stepContext.result;
    if (!bookingDetails.claimDOB) {
      const messageText = dob;
  ...
  async confirmStep(stepContext) {
    ...
    const messageText = "Hi Joe, Thanks for verifying. Is this for your Ford Model T ?";
    const msg = MessageFactory.text(
      messageText,
      messageText,
      InputHints.ExpectingInput
